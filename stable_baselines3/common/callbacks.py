...
from stable_baselines3.common.evaluation import evaluate_policy, evaluate_policy_aux
...
class EvalCallbackAux(EvalCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            episode_rewards, episode_lengths, aux_task_info = evaluate_policy_aux(
                self.model,
                self.eval_env,
                n_eval_episodes=self.n_eval_episodes,
                render=self.render,
            ...
            if self.verbose >= 1:
                print(f"Eval num_timesteps={self.num_timesteps}, " f"episode_reward={mean_reward:.2f} +/- {std_reward:.2f}")
                print(f"Episode length: {mean_ep_length:.2f} +/- {std_ep_length:.2f}")
                for k,v in aux_task_info.items():
                    print(f"Task {k} reward: {np.mean(v)}")
            # Add to current Logger
            self.logger.record("eval/mean_reward", float(mean_reward))
            self.logger.record("eval/mean_ep_length", mean_ep_length)
            for k,v in aux_task_info.items():
                self.logger.record(f"eval/mean_{k}", np.mean(v))
